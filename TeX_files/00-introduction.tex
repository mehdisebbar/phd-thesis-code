...
\begin{equation}
  J_{k\textnormal{-means}}(\bc_1,\dots,\bc_K)=\sum_{i=1}^N\min_{j\in [K]}\|\bx_i - \bc_j \|^2.
...
(Kmeans, loyds p 70 cluster analysis, difference k-means algo et kmeans objective, kmeans++, NP hard, mini batch kmeans fast)
...
\STATE {\bfseries Init:} Set $J_{\textnormal{old}}=\infty.$ and chose $K$ seed points $\bc_1,\dots,\bc_K$. Compute the $K$-means cost $J_{\textnormal{curr}}$ given in \cref{kmeans_min_problem} with these points as centers.
\WHILE{$J_{\textnormal{curr}}  < J_{\textnormal{old}}$} 
...
\end{equation}
\STATE{\tt 3: Set $J_{\textnormal{old}} = J_{\textnormal{curr}}$ and compute the new $K$-means cost $J_{\textnormal{curr}}$ with $\hat\bc_1,\dots,\hat\bc_K$ as centers.}
\ENDWHILE
\end{algorithmic}
\end{minipage}
}
...
\vspace{-15pt}
\end{figure}
Note that the choice of the cluster sample mean as center of the cluster is deeply connected to the choice of the Euclidean since it minimize the $K$-means objective.
The reader can refer to [Voronoi partition algorithm papers] for getting more info. Note that the 
