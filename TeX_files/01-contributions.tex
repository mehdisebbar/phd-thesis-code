...
In the first scenario, with the scaled identity matrix as precision matrix, the estimation error $\|\hat\bOmega_k-\bOmega^*\|_F$ of our algorithm is slightly better than EM, as shown in \Cref{fig:glasso_res_simu} and \Cref{fig:glasso_res_simu2}. The increase of the dimension accentuates the gap between EM and our algorithm. However, the increase of the number of clusters does not have a large impact on the error.
\begin{figure}
\centering
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
We note $\bTau_{.,k}$ the $k^{th}$ column and $\bTau_{i,.}$ the $i^{th}$ row of $\bTau$. Using the cost function in \Cref{cost_gmm_em} and adding a penalization term composed of the $L_2$-norms of the columns of $\bTau$ with a parameter $\lambda > 0$, we have the following cost function for our problem:
\begin{align*}
...
\STATE {(a) $\bTau^k =\argmin_{ \bTau : \forall i, \bTau_{i,.} \in \bPi}\big( \| \bTau - (\bxi^k - \eta \nabla f(\bxi^k) ) \|^2_2 \big)$,}
...
\subsection{Second method: penalizing the weight vector}
...
\STATE {\bfseries Output:} Parameter estimate $\hat\balpha = \big(\balpha_1,...,\balpha_{K-1},(1-\sum_{j=1}^{K-1}\balpha_j^{\gamma})^{\nicefrac1{\gamma}}).$
