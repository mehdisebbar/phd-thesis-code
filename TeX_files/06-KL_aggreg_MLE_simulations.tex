...
A closer look on the estimated weights by AD and MLE is necessary to observe the sparse estimation and the reasons of the poor performance of AD observed. We plotted the estimators of the weights when the target density is
$f_{\textnormal{gauss}}$ and $f_{\textnormal{gauss-lapl}}$. The dictionary used for this study is $D_{GLU}$. We started by $f_{\textnormal{gauss}}$, the estimated weights of the components of this density are plotted in \Cref{fig:weights_gauss_real_indexes}. We invite the reader to check on the table at \Cref{table:densities_DGLU} to see the components corresponding to the indexes. We observe that MLE provides accurate estimations of the weights of each components of the mixture, we recall that they are set to 0.2 in $f_{\textnormal{gauss}}$. The estimations given by AD are less accurate. It turns out that AD is more influenced by the uniform densities that are overlapping the components of the mixture as shown in \Cref{fig:weights_gauss_unif_indexes}. The sparse estimation of the weights can be observed by looking at components not used in the mixture $f_{\textnormal{gauss}}$, we can see in \Cref{fig:weights_gauss_other_indexes} that both AD and MLE provide sparse solution as expected. 
\begin{figure}
\center
    ...
    \caption{Estimated weights of the Gaussian components of $f_{\textnormal{gauss}}$, with $N=500$ (upper panel) and $N=1000$
    (lower panel).}
...
\end{figure}
The same plots for the density $f_{\textnormal{gauss-lapl}}$ are given in \Cref{fig:weights_gauss_laplace_real_indexes,fig:weights_gauss_laplace_unif_indexes,fig:weights_gauss_laplace_other_indexes}. 
When we look at the estimated weights of components of the density $f_{\textnormal{gauss-lapl}}$, AD estimated only 2 non zero components. A deeper look at the estimated weights of densities that have the same mean as the first component $\varphi(0, 10^{-2})$ of $f_{\textnormal{gauss-lapl}}$ reveals (see \Cref{fig:weights_gauss_laplace_around2_indexes}) that AD selected the uniform density (index 54) only. 
\begin{figure}
\center
...
\end{figure}
As a matter of fact, the estimated weight vector by AD is more sparse that MLE, but AD is more prone to be influenced by overlapping densities.\todos{verifier cette explication}. We performed a similar study for a better weighted density by AD at index 15 corresponding to $\varphi(0.6, 10^{-3})$ (see \Cref{fig:weights_gauss_laplace_around_indexes}). Again, the solution of AD is more sparse but this method is also influenced by the overlapping uniform density. The MLE gives more weight to the right components. However, the density at index 36 corresponding to the Laplace$(0.4, 0.2)$ of $f_{\textnormal{gauss-lapl}}$ is completely avoided by both methods (see \Cref{fig:weights_gauss_laplace_around3_indexes}). It seems that both has selected a mixture of $\varphi(0.4, 10^{-3})$ (index 11) and the two uniforms on $[0.3, 0.4]$ and $[0.4, 0.5]$ (indexes 57 and 58) instead of the Laplace$(0.4, 0.2)$. This is not surprising since this mixture is very close to the true component (see \Cref{fig:laplaceVSgauss_uniform}).
\begin{figure}
\center
...
\end{tabular}
\caption{Indices and components of the dictionary $D_{GL}$ and $D_{GLU}$ }
